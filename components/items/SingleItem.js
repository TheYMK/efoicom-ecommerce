        ...
        if (user && user.token) {
            addItemToWishlist(user.token, item._id)
                ...
                .catch((err) => {
                    ...
                    if (lang === "fr") {
                        toast.success(
                            `Oops! Echec de l'opération. Veuillez réessayer !`
                        );
                    }
                    if (lang === "en") {
                        toast.success(
            ...
            if (lang === "fr") {
                toast.success(
                    `Oops! Vous devez être connecter pour pouvoir ajouté un article dans vos favoris.`
                );
            }
            if (lang === "en") {
                toast.success(
                    "Oops ! You must be connected first before adding an item to your favorites."
                );
            }
        }
    };
            ...
            <div className="product-card product-card-profile">
                ...
                <div className="product-card-body ">
                    ...
                    <p className="text-muted mt-3">
                        ...
                        {item.reference_zone &&
                            item.reference_zone.name.charAt(0).toUpperCase() +
                                item.reference_zone.name.slice(1)}{" "}
                        (
                        {item.reference_zone &&
                            item.reference_zone.island.toUpperCase()}
                        )
                    </p>
                    <Link href={`/item/${item.slug}`}>
